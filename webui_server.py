...
async def api_delegated_playground_job(request):
    ...
    try:
        ...
        try:
            ...
            logs = {"stdout": "", "stderr": ""}
        return JSONResponse({"job": job, "events": events.get("events") or [], "artifacts": artifacts.get("artifacts") or [], "report": report, "logs": logs})
    except KeyError:
        return JSONResponse({"error": "delegated playground not found"}, status_code=404)
    except Exception as exc:
        return JSONResponse({"error": str(exc)}, status_code=500)
...
async function pollJob(){if(!currentDelegatedId||!currentJobId)return;try{const res=await fetch(`/api/delegated-playground/${encodeURIComponent(currentDelegatedId)}/job/${encodeURIComponent(currentJobId)}`);const data=await res.json();if(!res.ok)throw new Error(data.error||'读取 Job 失败');diagBody.innerHTML=renderJob(data);const job=data.job||{};const st=(job.status||'').toLowerCase();if(turns.length&&turns[turns.length-1].role==='agent'&&!['completed','failed','timeout'].includes(st)){const lastEvent=(data.events||[]).slice(-1)[0]||{};turns[turns.length-1]={role:'agent',text:`Job：${job.job_id||currentJobId}\n状态：${job.status||'-'}\n摘要：${job.summary||''}\n最新事件：${lastEvent.message||lastEvent.type||'等待更新'}${st==='running'?'\n\n真实 open_claude 可能正在执行或等待交互，请看右侧 stdout/stderr。':''}`,pending:true};renderChat();status('Job '+(job.status||'-')+'：'+(job.summary||''));}if(['completed','failed','timeout'].includes(st)){clearInterval(pollTimer);pollTimer=null;const report=data.report||((job.result||{}).summary)||st;turns[turns.length-1]={role:'agent',text:report,pending:false,error:st!=='completed'};renderChat();status('Job 结束：'+st);}}catch(e){diagBody.innerHTML='<div class="card"><h3>读取 Job 失败</h3><pre>'+esc(e.message||e)+'</pre></div>';}}
...
function renderOpenClaudeProcess(events,logs){const runnerEvents=events.filter(e=>String(e.type||'').startsWith('runner_'));const start=runnerEvents.find(e=>e.type==='runner_start')||{};const proc=runnerEvents.find(e=>e.type==='runner_process_started')||{};const outputs=runnerEvents.filter(e=>e.type==='runner_output').slice(-12);const heartbeats=runnerEvents.filter(e=>e.type==='runner_heartbeat').slice(-5);return `<div class="card"><h3>open_claude 执行过程</h3><div class="summary-grid"><div class="summary-row"><b>命令</b><span>${esc(((start.data||{}).command)||'未启动')}</span></div><div class="summary-row"><b>PID</b><span>${esc(String((proc.data||{}).pid||'-'))}</span></div><div class="summary-row"><b>工作目录</b><span>${esc(((start.data||{}).cwd)||'-')}</span></div><div class="summary-row"><b>超时</b><span>${esc(String(((start.data||{}).timeout_seconds)||'-'))} 秒</span></div></div>${heartbeats.length?`<details open><summary>运行心跳</summary>${eventTimeline(heartbeats)}</details>`:''}${outputs.length?`<details open><summary>最近输出事件</summary>${eventTimeline(outputs)}</details>`:'<div class="help">还没有捕获到 open_claude 输出。如果状态一直 running，可能是在模型请求、首次确认、或 CLI 无输出等待。</div>'}</div><details open><summary>stdout / stderr 原始日志</summary><h4>stdout.log</h4><pre>${esc(logs.stdout||'暂无 stdout 输出')}</pre><h4>stderr.log</h4><pre>${esc(logs.stderr||'暂无 stderr 输出')}</pre></details>`;}
function renderJob(data){const job=data.job||{},events=data.events||[],arts=data.artifacts||[],result=job.result||{},report=data.report||'',logs=data.logs||{};const cls=job.status==='completed'?'ok':(['failed','timeout'].includes(job.status)?'bad':'warn');const isRunning=job.status==='running';return `<div class="card"><h3>本轮结论</h3><div class="summary-grid"><div class="summary-row"><b>状态</b><span class="summary-status ${cls}">${esc(job.status||'-')}</span></div><div class="summary-row"><b>摘要</b><span>${esc(job.summary||result.summary||'')}</span></div><div class="summary-row"><b>Job</b><span>${esc(job.job_id||'')}</span></div><div class="summary-row"><b>Task Pack</b><span>${esc(job.task_pack_path||'-')}</span></div></div>${isRunning?'<div class="help">当前不是 queued，已经进入 running。若长时间不结束，通常是真实 open_claude 在执行、等待首次确认、模型网关卡住或没有输出。下面会显示 open_claude 命令、PID、心跳和输出。</div>':''}</div>${renderOpenClaudeProcess(events,logs)}<div class="card"><h3>Agent 回复 / report.md</h3><div class="md-body">${md(report||result.summary||'暂无 report.md，任务还没完成。')}</div></div><div class="card"><h3>产物</h3><div class="chips">${arts.length?arts.map(a=>`<span class="file">${esc(a.name||'')} · ${esc(String(a.size||0))} bytes</span>`).join(''):'<span class="small">暂无产物</span>'}</div></div><details><summary>全部过程 Events</summary>${eventTimeline(events)}</details><details><summary>完整 Job / Result JSON</summary>${section('Job','生成工程 /api/jobs/{job_id} 返回。',job)}${section('Result','artifacts/result.json 解析结果。',result)}</details>`;}
